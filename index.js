...
function generateQuestion() {
    ...
    <button type="submit" class="submitButton">Compute</button>
    </fieldset>
    </form>
    </div>`;
	} else {
	...
	function handleStartQuiz() {
		$('.quizStart').on('click', '.startButton', function(event) {
			$('.quizStart').remove();
			$('.js-quiz-container').css('display', 'block');
	...
	function renderResults() {
		if (score >= 8) {
			$('.js-quiz-container').html(`<div class="results correctFeedback"><h3>WOW! You are like, TOTALLY into 80's television!</h3><p>You got ${score} / 10</p><p>Ken Ober is applauding you from his grave. Now turn off Nick at Nite and go to bed already!</p><button class="restartButton">Restart Quiz</button></div>`);
		} 
		else if (score < 8 && score >= 5) {
			$('.js-quiz-container').html(`<div class="results correctFeedback"><h3>A TOTALLY fine score.</h3><p>You got ${score} / 10</p><p>Next time avoid the whammy!</p><button class="restartButton">Restart Quiz</button></div>`);
		} 
		else {
			$('.js-quiz-container').html(`<div class="results correctFeedback"><h3>TOTALLY bogus score dude.</h3><p>You got ${score} / 10</p><p>Maybe you'll like, do better on the 90's quiz?</p><button class="restartButton">Restart Quiz</button></div>`);
		}
	}
